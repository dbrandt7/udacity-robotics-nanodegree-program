...
def color_thresh(img, rgb_thresh=(160, 160, 160), rgb_thresh_rock_lower=(141, 114, 0), rgb_thresh_rock_upper=(213, 196, 100)):
    ...
    rock_thresh = ((img[:,:,0] >= rgb_thresh_rock_lower[0]) & (img[:,:,0] <= rgb_thresh_rock_upper[0])) \
                  & ((img[:,:,1] >= rgb_thresh_rock_lower[1]) & (img[:,:,1] <= rgb_thresh_rock_upper[1])) \
                  & ((img[:,:,2] >= rgb_thresh_rock_lower[2]) & (img[:,:,2] <= rgb_thresh_rock_upper[2]))
...
def perception_step(Rover):
    ...
    #threshed, rocks = color_thresh(warped)
    #obstacles = np.invert(threshed)

    threshed, rocks = color_thresh(img)
    #obstacles = np.invert(threshed)
    obstacles = 1-threshed
    ...
    temp_worldmap = np.zeros((200, 200))
    ...
    x_world_rocks, y_world_rocks = pix_to_world(xpix_rocks, ypix_rocks, Rover.pos[0],
                                                Rover.pos[1], Rover.yaw,
                                                Rover.worldmap.shape[0], scale)
    ...
    Rover.worldmap[y_world_rocks, x_world_rocks, 1] += 1

    # 8) Convert rover-centric pixel positions to polar coordinates
    # Update Rover pixel distances and angles
        # Rover.nav_dists = rover_centric_pixel_distances
        # Rover.nav_angles = rover_centric_angles
    distances, angles = to_polar_coords(x_world, y_world)
